...
@section('style')
    <style>
        ...
        #proposals-table_previous {
            display: none;
        }

        #proposals-table_next {
            display: none;
        }
    </style>
@endsection
                        ...
                        <tr>
                            ...
                            <td>
                                <span>
                                    @if ($proposal->project->status == 'CLOSED' && $proposal->project->winner_bidding_id == $proposal->id)
                                        <span class='badge rounded-pill bg-success px-3 py-2'><i class="fa fa-star"></i>
                                            WINNING BID</span>
                            ...
                            <td class="user-actions">
                                <a href="{{ route('client.proposals.show', $proposal) }}"
                                    ...
                                    <i class="fa fa-eye" style="color: #F96B23"></i>
                                </a>

                                @if ($proposal->project->winner_bidding_id == $proposal->id)
                                    <a href="{{ route('client.conversations.create', ['email' => $proposal->project->owner->email]) }}"
                                ...
                                @if ($proposal->project->status == 'ACTIVE')
                                    <button class="btn btn-sm btn-outline-danger btn-cancel-proposal"
                                        data-proposal='@json($proposal)'>
